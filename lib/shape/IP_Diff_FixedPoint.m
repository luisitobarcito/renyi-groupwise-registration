function [tX, A, W] = IP_Diff_FixedPoint(Xin, varargin)
...
if options.disp_progress
    ...
    for ob = 1:objNum
        ...
        plot(tX{ob}(:,1), tX{ob}(:,2), ['o',colmark(colidx)], 'MarkerSize',10, 'MarkerFaceColor', colmark(colidx), 'LineWidth', 3); hold on;
    end
...
for itr = 1:itrNum
	...
	IPtotCoeff = 2/((totalX^2)*(sig^2));
        ...
        newA{ob} = pinv(Aden)*Anum;
        newW{ob} = pinv(Wden)*Wnum;
